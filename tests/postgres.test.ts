...
describe("PostgreSQL credential grants", () => {
  ...
  test("rejects unsafe schema identifiers", () => {
    expect(credentialGrantsPostgresSchemaSql()).toContain(
      "secrets.credential_grants",
    );
    expect(() => credentialGrantsPostgresSchemaSql("bad;drop")).toThrow(
      "simple identifier",
    );
  });
});
