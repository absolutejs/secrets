...
describe('createSecretBroker', () => {
	...
	test('redact replaces every cached value in arbitrary text', async () => {
		...
		const log = 'GET /v1 failed with apikeyvalue1234567890 then DB error verysecretpassword end';
		const redacted = broker.redact(log);
		expect(redacted).toBe('GET /v1 failed with [REDACTED:API_KEY] then DB error [REDACTED:DB_PASSWORD] end');
	});
	...
	test('invalidate(name) clears one entry; invalidate() clears all', async () => {
		let fetches = 0;
		const broker = createSecretBroker({
			adapter: { fetch: async (name) => { fetches += 1; return `value-of-${name}-padded`; } },
		});
...
describe('compositeAdapter', () => {
	...
	test('put / rotate go to the first adapter that supports them', async () => {
		...
		const writeable: SecretAdapter = {
			fetch: async () => null,
			put: async (name) => { putTarget.push(name); },
	...
	test('list deduplicates across adapters', async () => {
		const a: SecretAdapter = { fetch: async () => null, list: async () => ['A', 'B'] };
		const b: SecretAdapter = { fetch: async () => null, list: async () => ['B', 'C'] };
		const composite = compositeAdapter([a, b]);
		expect(new Set(await composite.list?.() ?? [])).toEqual(new Set(['A', 'B', 'C']));
	});
});
